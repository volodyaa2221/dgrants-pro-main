class TrialPassthroughBudget < ActiveRecord::Base
  ...
  # Validations
  #----------------------------------------------------------------------
  validates_presence_of   :trial_schedule, :name
  validates_uniqueness_of :name, scope: :trial_schedule_id
  
  # Scopes
  #----------------------------------------------------------------------
  scope :activated_budgets,   -> {not_in(status: 0)}          # payable and editable budgets
  ...
  FIELD_HASH_ARRAY =  [ { name:   "mysql_trial_passthrough_budget_id",  type: "int(11)",          default: "NULL"},
  ...
  MYSQL_TABLE_NAME = Vpd::MYSQL_TABLE_NAME_PREFIX + "trial_passthrough_budgets"

  def self.data_to_mysql(vpd, connection)
    p ">>>>>>>>>>>>TrialPassthroughBudget Data To Mysql---------vpd : #{vpd.name}>>>>>>>>>>>>>>>"
    Vpd.create_mysql_table(connection, MYSQL_TABLE_NAME, FIELD_HASH_ARRAY)

    query_count=0 and queries=""
    data = TrialPassthroughBudget.where("vpd_id = #{vpd.id} AND (sync > 0 OR sync IS NULL)")
    data.each do |item|
      ...
      FIELD_HASH_ARRAY.each do |field|
        ...
        if field[:type] == "timestamp" || field[:type] == "datetime"
          ...
          if field[:name].include?("mysql_")
            if field[:name] == "mysql_trial_passthrough_budget_id"
              value_query  += "#{item.id}, "
      ...
      update_query = "UPDATE #{MYSQL_TABLE_NAME} SET #{update_query.from(0).to(-3)} WHERE `mysql_trial_passthrough_budget_id`=#{item.id}"
      if item.sync == 1
        results = connection.query("SELECT count(*) as count FROM #{MYSQL_TABLE_NAME} where `mysql_trial_passthrough_budget_id`=#{item.id};").first

  ...
  $.fn.rowReordering = function(options) {
    ...
    function fnGetState(sSelector, id) {
      ...
      if (trPrevious.length > 0) {
        iNewPosition = parseInt(oTable.fnGetData(trPrevious[0],
        ...
        if (trNext.length > 0) {
          iNewPosition = parseInt(oTable.fnGetData(trNext[0],
              properties.iIndexColumn));
    ...
    function fnMoveRows(sSelector, iCurrentPosition, iNewPosition, sDirection,
        id, sGroup) {
      ...
      $(oTable.fnGetNodes()).each(
          function() {
            ...
            if (iStart <= iRowPosition && iRowPosition <= iEnd) {
              if (tr.id == id) {
                oTable.fnUpdate(iNewPosition, oTable.fnGetPosition(tr), // get
                                                                        // row
                                                                        // position
                                                                        // in
                                                                        // current
                                                                        // model
                properties.iIndexColumn, false); // false = defer redraw until
                                                  // all row updates are done
              }
              else {
                if (sDirection == "back") {
                  oTable.fnUpdate(iRowPosition + 1, oTable.fnGetPosition(tr), // get
                                                                              // row
                                                                              // position
                                                                              // in
                                                                              // current
                                                                              // model
                  properties.iIndexColumn, false); // false = defer redraw
                                                    // until all row updates are
                                                    // done
                }
                else {
                  oTable.fnUpdate(iRowPosition - 1, oTable.fnGetPosition(tr), // get
    ...
    return this
        .each(function() {
          ...
          var aaSortingFixed = (oTable.fnSettings().aaSortingFixed == null ? new Array()
          ...
          for ( var i = 0; i < oTable.fnSettings().aoColumns.length; i++) {
            ...
            /*
             * for(var j=0; j<aaSortingFixed.length; j++) { if( i ==
             * aaSortingFixed[j][0] ) oTable.fnSettings().aoColumns[i].bSortable =
             * false; }
             */
          }
          oTable.fnDraw();
                ...
                update : function(event, ui) {
                  ...
                  if (properties.bGroupingUsed) {
                    ...
                    if (sGroup == null || sGroup == undefined) {
                      fnCancelSorting(tbody, properties, 3,
                          "Grouping row cannot be moved");
                      return;
                    }
                    sSelector = "tbody tr[" + properties.sDataGroupAttribute
                        + " ='" + sGroup + "']";
                  }
                  ...
                  if (properties.sURL != null) {
                    ...
                    $
                        .ajax({
                          ...
                          success : function() {
                            fnMoveRows(sSelector, oState.iCurrentPosition,
                                oState.iNewPosition, oState.sDirection,
                          ...
                          error : function(jqXHR) {
                            fnCancelSorting(tbody, properties, 1,
                                jqXHR.statusText);
                          }
                        });
                  }

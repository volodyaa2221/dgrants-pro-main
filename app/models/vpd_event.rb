class VpdEvent < ActiveRecord::Base
  self.inheritance_column = :_type_disabled

  # Constants
  #----------------------------------------------------------------------
  TYPE = {single_event: 0, patient_event: 1}
  TYPE_LABEL = ["Single Event", "Patient Event"]

  # Associations
  #----------------------------------------------------------------------
  belongs_to  :vpd          
  belongs_to  :dependency,        class_name: "VpdEvent"
  has_many    :subsequent_events, class_name: "VpdEvent", foreign_key: "dependency_id"
  has_many    :trial_events

  # Validations
  #----------------------------------------------------------------------
  validates_presence_of :event_id, :type
  validates_uniqueness_of :event_id, scope: :vpd_id
  ...
  # Callbacks
  #----------------------------------------------------------------------
  # after_save :update_trial_events
  ...
  # Public: Get maximum order of a vpd event to be created
  def self.max_order_in_fields(vpd, event_type)
    where(vpd: vpd, type: event_type).maximum(:order)
  end
  ...
  FIELD_HASH_ARRAY =  [ { name:   "mysql_vpd_event_id",             type: "int(11)",            default: "NULL"},
  ...
  MYSQL_TABLE_NAME = Vpd::MYSQL_TABLE_NAME_PREFIX + "vpd_events"

  def self.data_to_mysql(vpd, connection)
    p ">>>>>>>>>>>>VpdEvent Data To Mysql---------vpd : #{vpd.name}>>>>>>>>>>>>>>>"
    Vpd.create_mysql_table(connection, MYSQL_TABLE_NAME, FIELD_HASH_ARRAY)

    query_count=0 and queries=""
    data = VpdEvent.where("vpd_id = #{vpd.id} AND (sync > 0 OR sync IS NULL)")
    data.each do |item|
      ...
      FIELD_HASH_ARRAY.each do |field|
        ...
        if field[:type] == "timestamp" || field[:type] == "datetime"
          ...
          if field[:name].include?("mysql_")
            if field[:name] == "mysql_vpd_event_id"
              value_query  += "#{item.id}, "
      ...
      update_query = "UPDATE #{MYSQL_TABLE_NAME} SET #{update_query.from(0).to(-3)} WHERE `mysql_vpd_event_id`=#{item.id}"
      if item.sync == 1
        results = connection.query("SELECT count(*) as count FROM #{MYSQL_TABLE_NAME} where `mysql_vpd_event_id`=#{item.id};").first

class VpdCountry < ActiveRecord::Base
  ...
  # Callbacks
  #----------------------------------------------------------------------
  after_create :setup_vpd_country
  ...
  def trials
    trial_ids = Site.where(vpd_country: self, status: 1).map(&:trial_id).uniq
    Trial.where(id: trial_ids, status: 1)
  end

  # Public: Get all sites of this country in the given trial
  def sites_of_trial(trial)
    Site.where(vpd_country: self, trial: trial, status: 1)
  end

  # Public: Get real recruitment rate
  def real_recruitment_rate(trial)
    recruitment_rate = 0
    valid_sites      = 0

    self.sites_of_trial(trial).each do |site|
      events = SiteEvent.where("site_id = #{site.id} AND source != '#{SiteEvent::SOURCE[:forecasting]}' AND event_id = 'CONSENT' AND status = 1").order(happened_at: :asc)
      events_count = events.count
      next unless events_count > 1

      duration = (events.last.happened_at - events.first.happened_at).to_i
      duration = 1 if duration == 0
      recruitment_rate += events_count * 30.6 / duration
      valid_sites += 1
    end
    (recruitment_rate>0 && valid_sites>0) ? (recruitment_rate / valid_sites) : nil
  end
  ...
  # Private: Set name and code of country
  def setup_vpd_country
    self.update_attributes(name: country.name, code: country.code)
  end
  ...
  FIELD_HASH_ARRAY =  [ { name:   "mysql_vpd_country_id",       type: "int(11)",          default: "NULL"},
  ...
  MYSQL_TABLE_NAME = Vpd::MYSQL_TABLE_NAME_PREFIX + "vpd_countries"

  def self.data_to_mysql(vpd, connection)
    p ">>>>>>>>>>>>VpdCountry Data To Mysql---------vpd : #{vpd.name}>>>>>>>>>>>>>>>"
    Vpd.create_mysql_table(connection, MYSQL_TABLE_NAME, FIELD_HASH_ARRAY)

    query_count=0 and queries=""
    data = VpdCountry.where("vpd_id = #{vpd.id} AND (sync > 0 OR sync IS NULL)")
    data.each do |item|
      ...
      FIELD_HASH_ARRAY.each do |field|
        ...
        if field[:type] == "timestamp" || field[:type] == "datetime"
          ...
          if field[:name].include?("mysql_")
            if field[:name] == "mysql_vpd_country_id"
              value_query  += "#{item.id}, "
      ...
      update_query = "UPDATE #{MYSQL_TABLE_NAME} SET #{update_query.from(0).to(-3)} WHERE `mysql_vpd_country_id`=#{item.id}"
      if item.sync == 1
        results = connection.query("SELECT count(*) as count FROM #{MYSQL_TABLE_NAME} where `mysql_vpd_country_id`=#{item.id};").first
